...
window.onload = function() {
    ...
    calcForm.addEventListener('submit', function(e) {
        ...
        var monthly = 0,
            ...
            totalInterest = 0,
            invoice = 0;
        invoice = (parseFloat(units) *  0.20 + (parseFloat(days) *0.04 )); //calculating units + plus days
        vatInterest = parseFloat(invoice) * parseFloat(vat)  / 100; //calculating VAT value
        total = parseFloat(invoice) ; //calculate without VAR value
        totalInterest = parseFloat(invoice)  + parseFloat(vatInterest); //calculating total value
        setTimeout(() => {
